...
interface Props {
  id?: number;
  initItem?(item: DetailStorageItem): void;
}
...
export default class MainEditor extends Component<Props, State> {
  ...
  async componentDidMount() {
    if (this.props.id) {
      ...
      await this._init(true, this.props.id);
    }
  ...
  async _init(isEdit: boolean, id?: number) {
    ...
    try {
      ...
      if (isEdit && id) {
        // isEdit
        let item: DetailStorageItem = await fetchDetailItem(id);
        if (this.props.initItem) {
          this.props.initItem(item);
        }
      } else {
        // new create
        // this.setState({ isEdit: false, id: undefined });
      }
    } catch (err) {
      console.log(err);
  ...
  handleSubmit = async (value: FormValue) => {
    for (let [k, v] of Object.entries(value)) {
      if (v === "" || v === undefined || v === -1 || v === null) {
        showNotification(`${k} should not be empty`);
        return;
      }
    }
    let createdItem: DetailStorageItem | undefined = undefined;
    try {
      let client = new CreateAndupdater<PublishStorageItem>("item");
      this.setState({ progress: 0 });
      let publishItem: PublishStorageItem = {
        name: value.name,
        description: value.description,
        price: value.price,
        qr_code: value.qrCode,
        column: value.col !== null ? value.col : 0,
        row: value.row !== null ? value.row : 0,
      ...
      if (this.state.id) {
        createdItem = (await client.update(
          publishItem,
          this.state.id
        )) as DetailStorageItem;
      } else {
        createdItem = (await client.create(publishItem)) as DetailStorageItem;
      }
    } catch (err) {
      showNotification(err.toString());
    } finally {
      this.setState({ progress: undefined, redirect: true });
  ...
  render() {
    const { settings, redirect, item, id } = this.state;
        ...
        <AppBar position="sticky" elevation={0}>
          <Toolbar>
            <FormContext.Consumer>
              {({ clear }) => (
                <Link to="/">
                  <IconButton style={{ color: "white" }} onClick={clear}>
                    <ArrowBackIcon />
                  </IconButton>
                </Link>
              )}
              ...
              {({ formValue, clear }) => (
                <div className="mr-1 ml-auto">
                  <IconButton
                    color="inherit"
                    onClick={async () => {
                      await this.handleSubmit(formValue);
                    }}
                  >
                    <UploadButton />
                  </IconButton>
